...
class TimesFMAdapter:
    ...
    def predict_next(self, X_hist: pd.DataFrame, Y_hist: Optional[pd.DataFrame], input_len: int, lag: int = 1) -> tuple[np.ndarray, dict]:
        ...
        try:
            ...
            predictions = []
            for col in self.target_cols:
                series = series_df[col].values[-input_len:].astype(np.float32)
                # Create dataset for single series
                dataset = TimesFMSlidingWindowDataset(
                ...
                if len(dataset) == 0:
                    logging.warning(f"No valid windows for {col}; returning 0.0")
                    predictions.append(0.0)
                    continue

                # Get last window
                x_context, x_padding, freq, _ = dataset[-1]
                x_context = x_context.unsqueeze(-1)  # (context_length, 1)
                x_context = torch.tensor(self.x_scaler.transform(x_context.numpy()), dtype=torch.float32, device=self.device)
                x_padding = x_padding.to(self.device)
                freq = freq.to(self.device)

                # Predict
                with torch.no_grad():
                    output = self.model(x_context.unsqueeze(0), x_padding.unsqueeze(0), freq.unsqueeze(0))
                    pred = output[0, 0, 0].cpu().numpy()  # Select first step of first series
